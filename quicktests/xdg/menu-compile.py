"""
Patch to python-xdg to use lxml or cElementTree
"""
...
class Rule:
    ...
    def __init__(self, type, node=None):
        ...
        # Begin parsing
        if node:
            expr = self.parseRule(node)
            self.visitExpression(expr)
            self.Rule = compile(str(expr), '<compiled Rule>', 'eval')
...
if __name__ == "__main__":
    ...
    def iter_menu1(item):
        if isinstance(item, xdg.Menu.Menu):
            for child_item in item.getEntries(True):
                for child in iter_menu1(child_item):
                    yield child
        else:
            yield item
    ...
    def iter_menu2(item):
        if isinstance(item, Menu):
            for child_item in item.getEntries(True):
                for child in iter_menu2(child_item):
                    yield child
        else:
            yield item
    ...
    sys.path.insert(0, os.path.join(
        os.path.dirname(os.path.abspath(__file__)), '../../usr/lib/uxdgmenu'))
    import uxm.bench
    uxm.bench.step('Menu xdg.Menu + eval')
    #for i in xrange(100):
    menu1 = xdg.Menu.parse("kde4-applications.menu")
    for item in iter_menu1(menu1):
        continue
    uxm.bench.endstep('Menu xdg.Menu + eval')

    uxm.bench.step('Menu + OOP')
    menu2 = parse("kde4-applications.menu")
    for item in iter_menu2(menu2):
        continue
    uxm.bench.endstep('Menu + OOP')
