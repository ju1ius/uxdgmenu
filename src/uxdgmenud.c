...
static gchar *uxm_logfile_path = NULL;
...
{/*{{{*/
    ...
    uxm_logfile_path = uxm_get_logfile_path();

    /* Send g_message(  ) and g_warning(  ) to the file */
    g_log_set_handler(
        NULL,
        G_LOG_LEVEL_MESSAGE | G_LOG_LEVEL_DEBUG | G_LOG_LEVEL_WARNING | G_LOG_LEVEL_CRITICAL,
        uxm_log_handler,
        NULL
    );
    ...
    if(!formatter) {
        g_critical("No formatter provided - aborting");
        exit(EXIT_FAILURE);
    }
    ...
    g_message("Starting in %s", g_getenv("HOME"));
    ...
    listener_thread = g_thread_create(
        (GThreadFunc)uxm_monitor_listener,
        shared_data,
        TRUE,
        &error
    );
    if(!listener_thread) {
        g_critical("Error: %s - aborting", error->message);
        /*syslog(LOG_ERR, "Error: %s\n", error->message );*/
        exit(EXIT_FAILURE);
    }

    if(opts_flags & UXM_OPTS_WATCH_DEVICES) {
        udisks_thread = g_thread_create(
            (GThreadFunc)uxm_udisks_worker,
            shared_data,
            TRUE,
            &error
        );
    ...
    if(use_inotify) {
        inotify_thread = g_thread_create(
            (GThreadFunc)uxm_inotify_worker,
            shared_data,
            TRUE,
            &error
        );
...
{/*{{{*/
    ...
    if(!inotifytools_initialize()) {
        g_critical("Error: %s - aborting", strerror(inotifytools_error()));
    ...
    if(!inotifytools_ignore_events_by_regex(UXM_EXCLUDE_PATTERN, REG_EXTENDED)) {
        g_warning("Warning: Invalid inotify exclude pattern: %s", UXM_EXCLUDE_PATTERN);
        /*syslog(LOG_ERR, "Invalid exclude pattern: %s", UXM_EXCLUDE_PATTERN);*/
    } else if (verbose) {
        g_debug("Infotify: Ignoring pattern: %s", UXM_EXCLUDE_PATTERN);
    ...
    if(watch_applications) {
        ...
        for(iter = monitored; iter; iter = iter->next) {
            if(!inotifytools_watch_recursively(iter->data, UXM_APPS_EVENTS)) {
                g_warning("Cannot watch %s: %s", (char*)iter->data, strerror(inotifytools_error()));
                /*syslog(LOG_ERR, "Cannot watch %s: %s", (char*)iter->data, strerror(inotifytools_error()));*/
            } else if (verbose) {
                g_debug("Watching %s", (char*)iter->data);
    ...
    if(watch_recent_files) {
        ...
        if(recent_files_path == NULL) {
            ...
            g_warning("Recent files database not found");
            ...
            if(!inotifytools_watch_file(recent_files_dir, UXM_BOOKMARKS_EVENTS)) {
                watch_recent_files = 0;
                g_warning("%s: %s", recent_files_dir, strerror(inotifytools_error()));
                ...
                if (verbose) {    
                    g_debug("Watching %s", recent_files_dir);
    ...
    if(!inotifytools_get_num_watches()) {
        g_warning("Nothing to watch, exiting inotify thread...");
        goto end;
    }
...
{/*{{{*/
    ...
    if (proxy == NULL)
    {
        g_critical("Error creating udisks proxy: %s", error->message);
...
{/*{{{*/
    ...
    while(!uxm_stop_event) {
        ...
        if(l > 0) {
            ...
            g_snprintf(
                command_buf,
                UXM_UPDATE_CMD_BUF_SIZE,
                "%s -f %s",
                UXM_UPDATE_CMD_PREFIX,
                data->formatter
            );
            ...
            if (verbose) {
                g_debug("Launching command: %s", command_buf);
            }
            system(command_buf);
...
static void
uxm_log_handler(const gchar *log_domain, GLogLevelFlags log_level,
...
{/*{{{*/
    ...
    datetime = g_date_time_ref(g_date_time_new_now_local());
    fmt_date = g_date_time_format(datetime, "%F:%T");
    ...
    fp = fopen(uxm_logfile_path, "a");
    if (fp == NULL) {
        /* Fall back to console output if unable to open file */
        printf(format, fmt_date, message);
        return;
    }

    fprintf(fp, format, fmt_date, message);
    fclose(fp);
    g_free(fmt_date);
...
{/*{{{*/
    ...
    filepath = g_build_path("/", user_data_dir, "uxdgmenu", "uxdgmenud.log", NULL);
    return filepath;
}/*}}}*/
